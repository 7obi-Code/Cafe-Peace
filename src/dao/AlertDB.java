...
public class AlertDB implements AlertDBIF {
	...
	private static final String INSERT = "INSERT INTO Alert (type, description, severity, timestamp, productId_FK) VALUES (?, ?, ?, ?, ?)";
	...
	private static final String SELECT_ALL = "SELECT type, description, severity, timestamp FROM Alert"; // ingen ORDER
																											// BY tid
	...
	public List<Alert> getRecentAlerts() throws SQLException {
		...
		try (PreparedStatement ps = conn.prepareStatement(SELECT_ALL); ResultSet rs = ps.executeQuery()) {
			...
			while (rs.next()) {
				...
				// vi bruger stadig timestamp hvis du vil vise det senere, men vi checker ikke
				// “hvornår”
