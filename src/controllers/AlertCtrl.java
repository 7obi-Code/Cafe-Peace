...
public class AlertCtrl {
	...
	public Alert createAlert(Alert.Type type, String description, Alert.Severity severity, LocalDateTime timestamp,
	...
	// Checker om produkt er ramt lavt stock, og kaster en advarsel når den er ramt
	// minimum stock

	public boolean checkMinStock(Product p) throws DataAccessException {
		try {
			if (p.getStock().getAmount() < p.getMinStock()) {
				createAlert(Alert.Type.LOW_STOCK, p.getName() + " er nået lavt lager.", Alert.Severity.HØJ,
						LocalDateTime.now(), p);
				return true;
			}
			return false;
	...
	// Checker om produkt er ramt maximum stock, og kaster en advarsel hvis den er
	// ramt maximum stock
	public boolean checkMaxStock(Product p) throws DataAccessException {
		try {
			if (p.getStock().getAmount() > p.getMaxStock()) {
				createAlert(Alert.Type.MAX_STOCK, p.getName() + " er nået max grænsen for lager.", Alert.Severity.LAV,
						LocalDateTime.now(), p);
				return true;
			}
			return false;
	...
	public void checkDepositMatchInvoice(Invoice invoice, HashMap<Integer, Integer> countedQtyByProductId)
		...
		try {
			for (InvoiceLine il : invoice.getInvoiceLines()) {
				...
				if (invoiceQty != countedQty) {
					createAlert(Alert.Type.FAKTURA_FAILURE,
							"Det optalte antal af: " + il.getProduct().getName()
									+ " matcher ikke den forventede mængde: (" + countedQty + "/" + invoiceQty + ")",
							Alert.Severity.LAV, LocalDateTime.now(), il.getProduct());
				}
			}
		} catch (SQLException e) {
			throw new DataAccessException("Kunne ikke checke at den indtastede mængde matcher den forventede mængde.",
					e);
		}
	}
