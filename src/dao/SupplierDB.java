...
public class SupplierDB implements SupplierDBIF {
	...
	private static final String SELECT_ALL_SUPPLIERS = "SELECT phone, name, addressId_FK, email, cvr FROM Supplier";
	...
	private Supplier buildObject(ResultSet rs, boolean fullAssociation) throws DataAccessException {
		...
		try {
			if (rs.next()) {
				s = new Supplier(rs.getString("phone"), rs.getString("name"), rs.getInt("addressId_FK"),
						rs.getString("email"), rs.getInt("cvr"));
