...
public class InvoiceDB implements InvoiceDBIF {
	...
	private static final String SELECT_ALL_INVOICES = "SELECT invoiceNo, supPhone_FK, status, staff_FK FROM Invoice";

	private static final String SELECT_BY_INVOICENO = SELECT_ALL_INVOICES + " WHERE invoiceNo = ?";

	private static final String SELECT_ALL_INVOICELINES = "SELECT quantity, invoice_FK, product_FK FROM InvoiceLine";
	...
	public InvoiceDB() throws DataAccessException {
		...
		try {
			...
			selectInvoiceLinesByInvoiceNo = DBConnection.getInstance().getConnection()
					.prepareStatement(SELECT_INVOICELINE_BY_INVOICENO);
	...
	private Invoice buildObject(ResultSet rs, boolean fullAssociation) throws DataAccessException {
		...
		try {
			if (rs.next()) {
				i = new Invoice(rs.getInt("invoiceNo"), Status.valueOf(rs.getString("status")));
				if (fullAssociation) {
	...
	private ArrayList<InvoiceLine> buildInvoiceLines(int invoiceNo) throws DataAccessException {
		...
		try {
			...
			while (rs.next()) {
				...
				InvoiceLine il = new InvoiceLine(rs.getInt("quantity"), product);
				invoiceLines.add(il);
			}
